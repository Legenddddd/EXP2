...
class FGVCAircraft(Dataset):
    ...
    def __init__(self, root='/data2/zlj/Datasets/', train=True,
        ...
        self.IMAGE_PATH = os.path.join(root, 'fgvc-aircraft-2013b/data/images')
        self.SPLIT_PATH = os.path.join(root, 'fgvc-aircraft-2013b/data')
        if train:
            setname = 'train'
            csv_path = osp.join(self.SPLIT_PATH, 'images_variant_train.csv')
        else:
            setname = 'test'
            csv_path = osp.join(self.SPLIT_PATH, 'images_variant_test.csv')
        ...
        for l in lines:
            name, wnid = l.split(',')
            path = osp.join(self.IMAGE_PATH, name+'.jpg')
